...
export class EventListener {
    ...
    public async TokenLockEventListener(callback: (event: EventData) => Promise<void>) {
        for (const [chainId, config] of Object.entries(this.configs)) {
            ...
            bridgeContract.on('TokenLocked', async (token, sender, amount, recipient, sourceChainId, destinationChainId, event) => {
